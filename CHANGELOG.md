...
## Phase 31+ planned (NOT in this commit)

- **Phase 31 — SBI self-blending augmentation** (Face X-ray / SBI style): synthesise pseudo-fakes from HiDF REAL clips by blending a transformed copy of the face back onto itself → teaches generic blending-boundary artifacts. This is the literature-standard fix for cross-dataset transfer and the only credible path to CelebDF/FF++ ≥ 0.75 (P27 evidence: DANN over processing domains left FaceSwap at 0.381; P29 closed some gap — CelebDF 0.647 — but the ceiling without pseudo-fakes is plainly below 0.75). Real build: `data/HiDF_synthetic_generator.py` is currently a toy rectangle generator.
- **Phase 32 — SSL pretraining** on a mixed deepfake corpus (DFDC + FaceForensics++ unlabelled subsets) before fine-tuning on HiDF.
- **Phase 33 — Test-Time Adaptation (TTA)**: at eval time, run K=10 SGD steps on entropy minimisation per cross-dataset batch.
