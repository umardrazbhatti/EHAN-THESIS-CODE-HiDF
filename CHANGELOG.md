...
| 27 | 2026-06-11 | TBD | **Serial CBM** — out.logit IS cbm_logit; main_logit kept only as aux-supervision regulariser via `lambda_cbm_main_aux=0.05`; **K bumped 8 → 12** for serial expressiveness. **DANN domain adversarial training (DIAT variant)** — 4 synthetic domains via augmentation (clean / heavy-JPEG / noise / blur), per-sample random domain at `__getitem__`, GRL on attn_pool, DomainHead MLP (~34k params, ~0.7% of model). Linear warmup of `lambda_grl` 0→1.0 and `lambda_domain` 0→0.10 over 3 epochs. Detailed rolling-log + first-batch DIAG-P27 prints + 3 new history columns (`train_domain`, `train_domain_acc`, `train_cbm_main_aux`) for attribution. | TBD | TBD | TBD | TBD | TBD | **the gamble** |
